...
import { TapToHtml, Options, TapToVSError, LogFileHeader } from './index';
...
test('LogFileHeader', t => {

  const logFileHeader = new LogFileHeader('quhnbfleetmake/src');
  const value = logFileHeader.log('test');

  t.equal(value.substring(0, 41), '@quhnbfleetmake/src/../src/index.spec.ts(', 'logFileHeader');
  t.end();
});
